...
int __stdcall rtx2012_ConfFpRfLev (double RFLevel_dBm)
{
	...
	sprintf(wrtBuf,"CONF:FP:RF:LEV %f",RFLevel_dBm);
...
//	sprintf(wrtBuf,"CONF:MOD:DATA:TYPE %s",IeeeSyntax[dataTypeInBField]);
	sprintf(wrtBuf,"CONF:BER:DATA %s",IeeeSyntax[dataTypeInBField]);  
...
//	sprintf(wrtBuf,"CONF:TEST %s",IeeeSyntax[DUTType]);
	sprintf(wrtBuf,"CONF:TEST:TYPE %s",IeeeSyntax[DUTType]); 
...
int __stdcall rtx2012_ProcSelTest (int mode)
{
	...
	sprintf(wrtBuf,"PROC:SEL:TEST %s",IeeeSyntax[mode]);
	...
	if(i != 1)
		...
		sprintf(wrtBuf,"CONF:TEST NOP0");
	
	
	
	
	i = rtx2012_Write(wrtBuf);
	if(i != 1)
		return 0;
