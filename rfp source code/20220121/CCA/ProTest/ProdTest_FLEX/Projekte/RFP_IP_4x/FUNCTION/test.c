...
{
	long lSize; //int iM,iD,iY;

	GetFileSize ("C:\\DHCP_Server\\wwwroot\\iprfp4G.dnld", &lSize);
	//GetFileDate ("C:\\DHCP_Server\\wwwroot\\iprfp4G.dnld", &iM,&iD,&iY);
	
	
	if(!strncmp(strSerial,"3E7",3))
	{	 //telefonica   8.2SP1-FK13
		if(lSize != 36720495 /*|| iM != 6|| iD != 5|| iY != 2019*/) 
			act_structptr->resultstatus = check_value(-1);  
		else    act_structptr->resultstatus = check_value(0); 
	}
	else if(!strncmp(strSerial,"3L9",3))
	{	 //Egypt   8.3SP1-GI15
		if(lSize != 37569716  )
			act_structptr->resultstatus = check_value(-1);  
		else    act_structptr->resultstatus = check_value(0); 
	...
	{
		if(lSize != 36009829 /*|| iM != 11|| iD != 20|| iY != 2018*/) 
			act_structptr->resultstatus = check_value(-1);  
		else    act_structptr->resultstatus = check_value(0); 
	}
...
// set OC8, Relay for 25MHz
	...
	else
	{
		...
		for ( retry=0 ; retry < 3 ; retry++ )
		{
			if(!TRIGGER_HP53131A_TO_MEASURE ( &dResultFreq))
			{
				dResultFreq=-1;
				continue;
			}
...
// save the last Value
	...
	//  if((CMD60_ConfAfmForm (gpib_handle.cmd60Cnt, LT30Mhz, RMS)) != 0 ) return(0x0204);
		if(ipRfp_rsx_SetClock (0x01)) return(0x051f);
		Sleep(200);  //200ms
		if(!TRIGGER_HP53131A_TO_MEASURE ( &dBMC_Clock_min)) return(0x0205);

		if(ipRfp_rsx_SetClock (0xFF)) return(0x0520);
		Sleep(500);  //500ms
		if(!TRIGGER_HP53131A_TO_MEASURE ( &dBMC_Clock_max)) return(0x0206);
...
// zur Sicherheit 12x versuchen bis Timeout kommt
		for ( i = 0 ; i < 12 ; i++ )
		{
			...
			Sleep(300);  //500ms
#ifdef COUNT_LONG_WAY
			if(!TRIGGER_HP53131A_TO_MEASURE_Addon ( &dFreq)) return(0x0208);
#else
			if(!TRIGGER_HP53131A_TO_MEASURE ( &dFreq)) return(0x0208);
#endif
// dFreqdiff muss immer positive sein
			dFreqDiff = dFreq - dFreqMiddle;		//dFrwqMiddle is the otimal Value (center Frequency, (lowerLimit+upperLimit)/2)
			if (dFreqDiff < 0) dFreqDiff *= -1;       //nur positive
...
//CMD-setup:	
// Set Fixed Part 
	if(!rtx2012_ConfTest(FP)) return(0x020b);
//set Antenna 0 (CMD send this to RFP)	
	if(!rtx2012_ConfFpAnt (0)) return(0x20c);	
	if(ipRfp_rsx_SetDiversity(0x01)) return(0x0525);	  //sollte J33 sein
// Loopback Mode	
	if(!rtx2012_ConfFpSignMode(LOOPBACK)) return(0x020d);
//set packet type PP32Z (prolonged preamble)
	if( !rtx2012_addon_SetPacketTypePP32Z()) return(0x023e);
// RF-Level:
	dRfLevel = -45.0 + dHfOffset;
	if(dRfLevel > -40)	dRfLevel = -40;
	if(!rtx2012_ConfFpRfLev(dRfLevel)) return(0x020e);
//Traficslot + -carrier

	if(!rtx2012_ConfFpChannels(iTrafficslot, iTrafficcarrier, dCarrierOffset)) return(0x020f);
// loop back data type : 00001111 
	if(!rtx2012_ConfModDataType(FIG2)) return(0x0210);
//set Antenna 0 (CMD send this to RFP)	
	if(!rtx2012_ConfFpAnt (0)) return(0x20c);	
	
// set Bursts for NTP and MAX/MIN modulation (1..12000 default=4)
	if(!rtx2012_ConfAverBurs  (4)) return(0x0211);
//set Bursts for Timing  (2..2000  default=32)
	if(!rtx2012_ConfTimAverBurs (32)) return(0x0212);
//set packet type PP32Z (prolonged preamble)
	if(!rtx2012_addon_SetPacketTypePP32Z()) return(0x0213); //[@JM080918]
//set Antenna   0 or 1
	if(!rtx2012_ConfFpAnt(0)) return(0x0214);   // default after reset is 0
//set "Radio Fixed Part Identity"
	if(!rtx2012_ConfFpRfpi(szRFPI) ) return(0x0215);
// Manual Mode 
	if(!rtx2012_ProcSelTest(MANUAL)) return(0x0216);
// setup connection 

	if(!rtx2012_addon_Connect( /*function*/0, cErrorInfo)) return(0x02FF); 
...
// set Ber Eval Window: 100 Bursts
	...
	{
		if(!rtx2012_ConfBerEvalWind (100)) return(0x0241);
		if(!rtx2012_ConfBerRfLev (dBerRfLevel)) return(0x0242);

		if(!rtx2012_ReadBerLter (&dBER, &dFER)) dBER = 9999;	 // bei NAN  dBERR auf 9999
		else changeToPPM(&dBER, &dFER);
	}
...
//RSSI Dämpfung
	...
	else
	{
		//rtx2012 - setup:
// loop back data type : 00001111
		if(!rtx2012_ConfModDataType ( FIG2)) return(0x0223);
// set Bursts for NTP and MAX/MIN modulation (1..12000 default=4)
		if(!rtx2012_ConfAverBurs  ( 4)) return(0x0224);
// RF-Level:
//RSSI Dämpfung
		dRssiAttenuation = -2.0;         //		experimental,
		dRfLevel = -45.0 + dHfOffset + dRssiAttenuation;
		if(dRfLevel > -40)	dRfLevel = -40;
		if(!rtx2012_ConfFpRfLev ((dRfLevel))) return(0x0225);
	}
...
//RSSI Dämpfung
	...
	{
		if(!rtx2012_ConfFpRfLev (( dRfLevel ))) return(0x0229);    
	}
...
// RF-Level:
	dRfLevel = -45.0 + dHfOffset;
	if(dRfLevel > -40)	dRfLevel = -40;
	if(!rtx2012_ConfFpRfLev( dRfLevel)) return(0x022b);
//reset to default values for Power Ramp and NTP.
	if ( !rtx2012_CalcLimPowTranTempCle () )  return(0x022c);
	/*
	** Remark: Please configure CMD60/65 Escape-Sequence (via Config-Key) with Data: 7000000000 (Receiver)
	** Configure Power-Ramp Packet Type: PP32 (Transmitter)
	*/
//set packet type PP32Z (prolonged preamble)
	if( !rtx2012_addon_SetPacketTypePP32Z()) return(0x022d); //[@JM080918]

#ifdef RFP35_BER_WITH_ESC_SEQUENCE
// muss nach connection
// activate SW-demod for BER
// CONF:MESS:ESC:MT "EE60010000"
	if ( (!rtx2012_addon_activate_SW_Demod())) return(0x0221);
#endif //RFP35_BER_WITH_ESC_SEQUENCE
// loop back data type : 00001111
	if(!rtx2012_ConfModDataType ( FIG2)) return(0x022e);
// set Bursts for NTP and MAX/MIN modulation (1..12000 default=4)
	if(!rtx2012_ConfAverBurs  ( 4)) return(0x022f);
//set Bursts for Timing  (2..2000  default=32)
	if(!rtx2012_ConfTimAverBurs ( 32)) return(0x0230);
// ntp min max Werte setzen sind nur Dummy Werte
	if(!rtx2012_CalcLimPowTranTempTol ( 10.0 , 30.0 )) return(0x0231);
	for ( retry=5, verdictForPowerRamp=9 ; verdictForPowerRamp != 0 && retry > 0 ; retry-- )
	{
// die ersten zwei Messungen werden benötigt dammit
// eine PowerRamp Messung erzeugt wird
...
// Normal ntp lesen
		//if(CMD60_addon_ReadNormalTransmitPower (gpib_handle.cmd60,&dNTPDummy)) return(0x0232);
		if(!rtx2012_addon_ReadNormalTransmitPower (&dNTPDummy)) continue;  //fail NaN  [@GC 10.05.2012]
// Verdict NTP lesen
		if(!rtx2012_CalcLimPowTranTolMatc ( &verdictForNTPDummy)) return(0x0233);
		// <0> : MATC | measurement does not exceed the tolerance.
		// <1> : NMAT | measurement exceeds the tolerance.
		// <2> : INV  | no measurement result available.
		// dies ist die PowerRamp Auswertung
		if(!rtx2012_CalcLimPowTranMatc (&verdictForPowerRamp)) return(0x0234);
		//        <0> : MATC | signal matches template.
		//        <1> : NMAT | signal does not match template.
		//        <2> : INV  | no measurement result available.
	}
...
char linkStatus[256];
	...
	sprintf(mail_buffer,"manu version\n");
	if(ipRfp_rs232_mail_out(mail_buffer,pExpected,NULL))    // waits 5 second for answer
		{
	 	set_hintwindow("Test software version check Error!",COL_NEUTRAL,(testflow->running_left ? LEFT : RIGHT));	  // LEFT, RIGHT     
		act_structptr->resultstatus = check_value(-1);
		}
	else
		{
	 	set_hintwindow("Test software version check OK!",COL_NEUTRAL,(testflow->running_left ? LEFT : RIGHT));	  // LEFT, RIGHT     
		act_structptr->resultstatus = check_value(1);
		}
...
{
	if(ipRfp_rs232_sendBootFlashImage())
		act_structptr->resultstatus = check_value(-1);
