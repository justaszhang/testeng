...
double dTemperature=1;
	...
	if ( ipRfp_libssh_getTemperature(&dTemperature) ) return(0x0507); 
	act_structptr->resultstatus = check_value(dTemperature);
	return(0);   
}
...
//	if (ipRfp_rsx_SetFrequencyBand (iFrequencyBand)) return(0x0527);
//	if (ipRfp_rsx_SetTransmitPower(iPowerLevel)) return(0x0528);

//	if (ipRfp_rsx_SetupDummyBearer(iDummycarrier, iDummyslot)) return(0x052a); 
//CMD-setup:	
...
//
//RSSI Dämpfung
	dRssiAttenuation = -2.0;         // -2.0dBm   (der Wert muss negative sein da es dämpfen soll)
//falscher Parameter
	if ( rfpart < 1 || rfpart > 2 ) return(0x200c);
//assign HF-offset, traficcarrier and -slot dependent from radio
...
//RSSI Dämpfung
	dRssiAttenuation = -2.0;         // -2.0dBm   (der Wert muss negative sein da es dämpfen soll)
//falscher Parameter
	if ( rfpart < 1 || rfpart > 2 ) return(0x200d);
//assign HF-offset, traficcarrier and -slot dependent from radio
	if (1 == rfpart)    //RF-Part1
	{
		dHfOffset = dHFOffsetRF1;
