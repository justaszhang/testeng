...
unsigned char wlanResultStr[0x8000];
...
// setup CLIENT MODE: (is stred in EEPROM, after power up this mode is activ, however, setup again ..
...
{
int j,i = 0, errorCount = 0, loopCount = 2;
char	*chrptr;

	do
	{
		FlushInQ (WLanEmbedded_Port1);	
		FlushInQ (WLanEmbedded_Port2);	
		FlushInQ (WLanEmbedded_Port3);	
...
*/	
		...
		{// gefunden
			...
			errorCount |= 4;
// 20181011, patch, some modules not responding, reset the module and try again
		
		if(errorCount)
		{
			if(errorCount & 0x01)
				i+= WLanEmbedded_write (WLanEmbedded_Port1, "AT+RST");
			if(errorCount & 0x02)
				i+= WLanEmbedded_write (WLanEmbedded_Port2, "AT+RST");
			if(errorCount & 0x04)
				i+= WLanEmbedded_write (WLanEmbedded_Port3, "AT+RST");
			errorCount = 0;
			Sleep(1000);
		}
		else  // errorCount == 0, success :)
			break;
	}while(--loopCount);
