...
/*
20181218, look some lines later
...
//20181218, Dietmar, add Reset via ComEscape and hardware patch
int MiniSys_getLightSensor (void)
{
int iResult, loopCount = 2;
   ...
   do
   {
		...
		if(!MiniSys_read (ResultStr))
   		...
   		iResult = atoi(ResultStr);
   		if((iResult > 0xff00) ||(iResult < 0xff))
   		{
// reset MiniSys via FTDI DTR toggle, PATCH on MiniSys: ZVN2106 on *RST and GND, Gate wired to FTDI232 Pin2 = *DTR
/*
...
*/	
			ComSetEscape (MiniSysPort, 6);		// clr *DTR  --> DTR goes high 
			Sleep(50);
			ComSetEscape (MiniSysPort, 5);		// set *DTR  --> DTR goes low, MiniSys restart 
			Sleep(900);
	   		MiniSys_write ("RGB");
	   		Sleep(400);							// first time init after reset
   		}
   		else
   			break;		 // iResult below 0xff00
   	}while(--loopCount);
	return(iResult);
}
