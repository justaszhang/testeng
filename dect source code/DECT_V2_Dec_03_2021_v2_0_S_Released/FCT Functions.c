...
double test_time = 0;  
	...
	if(GetFileInfo ("612d_CCA.TXT", &size)){ 
		...
		SetDir(".."); 
			...
			H_TRACS = OpenFile ("612d_CCA.TXT", VAL_READ_WRITE, VAL_APPEND,VAL_ASCII);
			//create title block
			sprintf(fault_message,"Part Number,Serial Number,Fixture Serial,Pass_Fail,Start Date,Start Time,Finish Date,Finish Time,Test Time,GUI_Revision,InsCurr,LeakCurr,PwrOnCurr,Test Mode,CharCurr_dc,Error Counter, P1V8 Adjustment,"
					"P1V8 Voltage,P3V,Ref Clk, Ref Clk Adjustment, Ref Clk Freq, Branding Set, Branding Check, HW Release, Write IPEI, Write SN,  Write Full SN, Reset Test Flow, File System, Open CMD60, CMD60 Loopback," 
					...
					"RSSI K Factor, RSSI Offset, Receiver Sensitivity, Setup Power Match, Power Match, Close CMD60, EEPROM LED, EEPROM Date, EEPROM PASS, Make Backup, Power Off\n");
			buff_size = strlen(fault_message);		
			WriteFile (H_TRACS, fault_message, buff_size);
	...
	if(GetFileInfo ("622d_CCA.TXT", &size)){ 
		...
		SetDir(".."); 
			...
			//create title block
			sprintf(fault_message,"Part Number,Serial Number,Fixture Serial,Pass_Fail,Start Date,Start Time,Finish Date,Finish Time,Test Time,GUI_Revision,InsCurr,LeakCurr,PwrOnCurr,Test Mode,CharCurr_dc, Error Counter, P1V8 Adjustment,"
					"P1V8 Voltage,P3V, Ref Clk, Ref Clk Adjustment, Ref Clk Freq, Branding Set, Branding Check, HW Release, Write IPEI, Write SN, Write Full SN,Reset Test Flow, File System, SD Card, Open CMD60, CMD60 Loopback," 
					"SW Version, Park Search, PLI_1, Park_1, Test Mode, Service State, Connect CMD60, NTP, POS EU Deviation, NEG EU Deviation, Freq Offeset, Freq Drift, RSSI 45dBm, RSSI 75 dBm,"
					"RSSI K Factor, RSSI Offset, Receiver Sensitivity, Setup Power Match, Power Match, Close CMD60, Backup, Read MAC, Test Mode, Bluetooth Test Mode, Bluetooth Test Mode,CharCurr_L,CharCurr_H,CharCurr_N, Open MT8852B, Sync MT8852B, MT8852B Loopback Mode," 
					"BT Counter, MT8852B Scripts, BT Output Power, BT Mod Index, BT Frequency Tolerance, BT Frequency Drift, BT Slot Sensitivity, BT Sync Clock, BT Get Trim, BT Write Trim, Close MT8852B,"
...
if(Test_Type == 2){
	...
	if(GetFileInfo ("632d_CCA.TXT", &size)){ 
		...
		SetDir(".."); 
			...
			//create title block
			sprintf(fault_message,"Part Number,Serial Number,Fixture Serial,Pass_Fail,Start Date,Start Time,Finish Date,Finish Time,Test Time,GUI_Revision,InsCurr,LeakCurr,PwrOnCurr,Test Mode,CharCurr_dc, Error Counter, P1V8 Adjustment,"
					"P1V8 Voltage,P3V, Ref Clk, Ref Clk Adjustment, Ref Clk Freq, Branding Set, Branding Check, HW Release, Man Down,Write IPEI, Write SN,Write Full SN, Reset Test Flow, File System, SD Card, Open CMD60, CMD60 Loopback," 
					"SW Version, Park Search, PLI_1, Park_1, Test Mode, Service State, Connect CMD60, NTP, POS EU Deviation, NEG EU Deviation, Freq Offeset, Freq Drift, RSSI 45dBm, RSSI 75 dBm,"
					"RSSI K Factor, RSSI Offset, Receiver Sensitivity, Setup Power Match, Power Match,Close CMD60, Backup, Read MAC, Test Mode, Bluetooth Test Mode, Bluetooth Test Mode,CharCurr_L,CharCurr_H,CharCurr_N, Open MT8852B, Sync MT8852B, MT8852B Loopback Mode," 
					"BT Counter, MT8852B Scripts, BT Output Power, BT Mod Index, BT Frequency Tolerance, BT Frequency Drift, BT Slot Sensitivity, BT Sync Clock, BT Get Trim, BT Write Trim, Close MT8852B,"
...
if(Test_Type == 3){
	...
	if(GetFileInfo ("612d_FINAL.TXT", &size)){ 
		...
		SetDir(".."); 
			...
			H_TRACS = OpenFile ("612d_FINAL.TXT", VAL_READ_WRITE, VAL_APPEND,VAL_ASCII);
			//create title block
			sprintf(fault_message,"Part Number,Serial Number,Fixture Serial,Pass_Fail,Start Date,Start Time,Finish Date,Finish Time,Test Time,GUI_Revision,Test Mode, Error Counter, Reset Test Flow, READ SN, SN Check,Full SN check, READ IPEI, IPEI Check, KEYB LED, KEY12, LCD POS,"
...
if(Test_Type == 4){
	...
	if(GetFileInfo ("622d_FINAL.TXT", &size)){ 
		...
		SetDir(".."); 
			...
			H_TRACS = OpenFile ("622d_FINAL.TXT", VAL_READ_WRITE, VAL_APPEND,VAL_ASCII);
			//create title block
			sprintf(fault_message,"Part Number,Serial Number,Fixture Serial,Pass_Fail,Start Date,Start Time,Finish Date,Finish Time,Test Time,GUI_Revision,Test Mode, Error Counter, Reset Test Flow, READ SN, SN Check,Full SN check, READ IPEI, IPEI Check, Read MAC, MAC Check, KEYB LED, KEY22, LCD POS,"
					...
					"MT8852B Loopback, BT Counter, MT8852B Scripts, Power MT8852B, Close MT8852B, Disable BT, Write Date, Write Pass, Make Backup, Master Reset, Power Off\n");
			buff_size = strlen(fault_message);		
			WriteFile (H_TRACS, fault_message, buff_size);
...
if(Test_Type == 5){
	...
	if(GetFileInfo ("632d_FINAL.TXT", &size)){ 
		...
		SetDir(".."); 
			...
			H_TRACS = OpenFile ("632d_FINAL.TXT", VAL_READ_WRITE, VAL_APPEND,VAL_ASCII);
			//create title block
			sprintf(fault_message,"Part Number,Serial Number,Fixture Serial,Pass_Fail,Start Date,Start Time,Finish Date,Finish Time,Test Time,GUI_Revision,Test Mode, Error Counter, Reset Test Flow, READ SN, SN Check,Full SN check, READ IPEI, IPEI Check, READ MAC, MAC Check,KEYB LED, KEY33, LCD POS,"
					"LCD RED, LCD GREEN, LCD BLUE, LED RED, LED GREEN, LED ORANGE, SD CARD, Audio Headset, Audio MIC, Audio Speaker, Audio EP, Audio Trembler, Open CMD60, CMD60 Loopback,"
					"SW Version, Park Search, PLI_1, Park_1, Test Mode, Service State, Connect CMD60, NTP, Close CMD60, Read MAC, Test Mode, BT Test Mode, BT Test Mode, Open MT8852B, Sync MT8852B,"
					"MT8852B Loopback, BT Counter, MT8852B Scripts, Power MT8852B, Close MT8852B, Disable BT,Write Date, Write Pass, Make Backup, Master Reset, Power Off\n");
			buff_size = strlen(fault_message);		
...
char tftp_BT_MAC_path[MAX_PATHNAME_LEN];  
			...
			memset(log_file_results,'\0', sizeof(log_file_results));  
...
int DECT_612d_CCA_Test_Sequence(int panel, char *test_number)
{
	...
	//init agilent
	if(!Init_Agilent (panel))
		return 0;

	// Set agilent for test leakage current
	if(!Set_Agilent_LeakCurr(panel))
		return 0;

	//Measure_Agilent_Insance Current
	if(!Measure_Agilent_InsCurr(panel) )
		return 0;

	//Measure_Agilent_Leakage Current
	if(!Measure_Agilent_LeakCurr(panel))
		return 0;
	...
	//Measure_3V Voltage
	if(!DECT_P3V_Voltage_Measure_Test(panel))
		return 0;
	...
	// measure power on current
	if(!Measure_Agilent_PwrOnCurr(panel))
		return 0;
	...
	//get charing status
	if(!DECT_GetChargerStatus(panel))
		return 0;


	//set agilent for charging
	if(!Set_Agilent_CharCurr(panel))
		return 0;
	...
	// measure charging high
	if(!Measure_Agilent_CharCurr(3,panel))
		return 0;

	//set DC off
	EIO3_Off();

	if(!Set_Agilent_NormalWorkVolt(panel))
		return 0;
...
/*	if(Error_Counter != 0){
	...
	if(0 == irfEquipmentType)	   //cmd60
	{
		...
		if(DECT_Get_SW_Version_Test_612 (panel) != 1)
		{
		...
		if(Set_CMD60_Mask_Setting(panel) != 1)
		{
			sprintf(test_number, "Set Template");
			return 0;
		}
	...
	else if(1 == irfEquipmentType)	   //RTX2012   
	{
		
		if(Open_RTX (panel) != 1)
		{
			sprintf(test_number, "Open_RTX");
			DECT_Clean_Shutdown_Sequence(panel);
			return 0;
		}
		...
		if(Set_RTX_in_Loopback_Mode_EU_Setting (panel) != 1)
		{
			sprintf(test_number, "RTX_Loopback");
			DECT_Clean_Shutdown_Sequence(panel);
				Close_RTX(panel); 
			return 0;
		}
		...
		if(Setup_Connect_RTX(panel) != 1)
		{
			sprintf(test_number, "Setup_Connect_RTX");
			DECT_Clean_Shutdown_Sequence(panel);
				Close_RTX(panel); 
			return 0;
		}
		...
		if(DECT_Measure_RTX_NTP_Power_Test(panel) != 1)
		{
			sprintf(test_number, "NTP_Power");
			DECT_Clean_Shutdown_Sequence(panel);
				Close_RTX(panel); 
		...
		if(DECT_Measure_RTX_EU_Deviation_Test(panel) != 1)
		{
			sprintf(test_number, "EU_Deviation");
			DECT_Clean_Shutdown_Sequence(panel);
				Close_RTX(panel); 
		...
		if(DECT_Measure_RTX_Frequency_Offset_Test(panel) != 1)
		{
			sprintf(test_number, "Freq_Offset");
			DECT_Clean_Shutdown_Sequence(panel);
				Close_RTX(panel); 
		...
		if(DECT_Measure_RTX_Frequency_Drift_Test(panel) != 1)
		{
			sprintf(test_number, "Freq_Drift");
			DECT_Clean_Shutdown_Sequence(panel);
				Close_RTX(panel); 
		...
		if(DECT_Measure_RTX_RSSI_45dBm_Test(panel) != 1)
		{
			sprintf(test_number, "RSSI_45dBm");
			DECT_Clean_Shutdown_Sequence(panel);
				Close_RTX(panel); 
		...
		if(DECT_Measure_RTX_RSSI_75dBm_Test(panel) != 1)
		{
			sprintf(test_number, "RSSI_75dBm");
			DECT_Clean_Shutdown_Sequence(panel);
				Close_RTX(panel); 
		...
		if(DECT_Measure_RTX_Receiver_Sensitivity_Test(panel) != 1)
		{
			sprintf(test_number, "Receiver Sensitivity");
			DECT_Clean_Shutdown_Sequence(panel);
				Close_RTX(panel); 
		...
		if(Set_RTX_Mask_Setting(panel) != 1)
		{
			sprintf(test_number, "Set Template");
				Close_RTX(panel); 
			return 0;
		...
		if(DECT_RTX_Power_Template_Match_Test(panel) != 1)
		{
			sprintf(test_number, "Template Match");
			DECT_Clean_Shutdown_Sequence(panel);
				Close_RTX(panel); 
		...
		if(Close_RTX(panel) != 1)
		{
			sprintf(test_number, "Close_RTX");
			DECT_Clean_Shutdown_Sequence(panel);
			return 0;
		}
	...
	{
		sprintf(test_number, "unsupport rf equipment");
		DECT_Clean_Shutdown_Sequence(panel);
		return 0;
	}
...
int DECT_622d_CCA_Test_Sequence(int panel, char *test_number)
{
	...
	if(0 == irfEquipmentType)	  //cmd60
	{
		...
		if(DECT_Get_SW_Version_Test_622 (panel) != 1)
		{
			sprintf(test_number, "SW_Version_Test");
			DECT_Clean_Shutdown_Sequence(panel);
			return 0;
	...
	{
			sprintf(msg,"unsupport rf equipment\n");
	...
	//get charing status
	if(!DECT_GetChargerStatus_USB(panel))
		return 0;

	//set low charging
	if(!DECT_SetChargeCurrentMode_0_USB(panel))
		return 0;

	// measure charging low
	if(!Measure_Agilent_CharCurr(1,panel))
		return 0;


	//set high charging
	if(!DECT_SetChargeCurrentMode_1_USB(panel))
		return 0;

	// measure charging high
	if(!Measure_Agilent_CharCurr(2,panel))
		return 0;

	//set disable charging
	if(!DECT_DisableCharger_USB(panel))
		return 0;

	// measure charging disable
	if(!Measure_Agilent_CharCurr(0,panel))
		return 0;
	...
	if(DECT_Bluetooth_Read_ClockAdjust_USB_Test(panel) != 1){
		sprintf(test_number, "Bluetooth_Read_ClockAdjust ");
		DECT_Clean_Shutdown_USB_Sequence(panel); 
		return 0;
	}
...
int DECT_632d_CCA_Test_Sequence(int panel, char *test_number)
{
	...
	if(0 == irfEquipmentType) //cmd60
	{
		...
		if(DECT_Get_SW_Version_Test_632 (panel) != 1)
		{
			sprintf(test_number, "SW_Version_Test");
			DECT_Clean_Shutdown_Sequence(panel);
			return 0;
...
int DECT_612d_Final_Test_Sequence(int panel, char *test_number)
{
	...
	//sprintf(msg,"ê??ˉ?a?ú￡?°′?°?úESC??è?à?é??á???￡??è?oó°′OK?￡\n\nPower on the Phone manually. Use the ESC key on the phone to enter Blue Screen.\n\n Press OK when Blue Screen is visible.");
	sprintf(msg,"手动开机，按话机ESC进入蓝色屏幕。进入后按OK.\n\nPower on the Phone manually and press OK only after phone has fully initialized and booted."); 
	Display_OK_MSG(msg); 
	...
	if(DECT_Check_Serial_Number_String_Test(panel) != 1)
	{
		sprintf(test_number, "SN_STR_CHECK");
		DECT_Clean_Shutdown_Sequence(panel);
		return 0;
	}
...
/*	if(DECT_Audio_Headset_Test(panel) != 1){
	...
	{
		if(DECT_Audio_Headset_Test (panel,iRepeat) == 1)
			break;
		else
		{
			if(iRepeat == 2)
	...
	//Audio Tests
	////////////////////////
	sprintf(msg,"关闭屏蔽箱后按OK\n\nClose Test Fixture Cover, then Press OK.");
	Display_OK_MSG(msg);
...
/*	if(DECT_Audio_Speaker_Test (panel) != 1){
	...
	if(DECT_Audio_TestReset_Test(panel) != 1)
	{
		sprintf(test_number, "Audio_TestReset");
		DECT_Clean_Shutdown_Sequence(panel);
		return 0;
	}
...
//	int iRepeat = 0;
	for(iRepeat =0;  iRepeat < 3; iRepeat++)
	{
		if(DECT_Audio_Speaker_Test_Ex (panel,iRepeat) == 1)
			break;
		else
		{
			if(iRepeat == 2)
	...
	for(iRepeat =0;  iRepeat < 3; iRepeat++)
	{
		if(DECT_Audio_EarpieceTest_Test_Ex (panel,iRepeat) == 1)
			break;
		else
		{
			if(iRepeat == 2)
	...
	if(DECT_Audio_TestPass_Test(panel) != 1)
	{
		sprintf(test_number, "Audio_TestPass");
		DECT_Clean_Shutdown_Sequence(panel);
		return 0;
	}
	...
	if(DECT_Get_HW_Version_Test (panel,0) != 1){
		sprintf(test_number, "HW_Version_Test");
		DECT_Clean_Shutdown_Sequence(panel); 
		return 0;
	}
	...
	sprintf(msg,"打开屏蔽箱后按OK\n\nOpen Test Fixture Cover, then Press OK.");
	Display_OK_MSG(msg);
...
/*	if(DECT_Audio_Speaker_Test (panel) != 1){
	...
	if (Stop_Test_Exe == 1)
	{
		...
		if(DECT_Trembler_TestReset_Test(panel) != 1)
	{
		sprintf(test_number, "Audio_TestReset");
		DECT_Clean_Shutdown_Sequence(panel);
		return 0;
...
//	int iRepeat = 0;
	...
	for(iRepeat =0;  iRepeat < 3; iRepeat++)
	{
		if(DECT_Audio_TremblerTest_Test_Ex(panel,iRepeat) == 1)
			break;
		else
		{
			if(iRepeat == 2)
	...
	if (Stop_Test_Exe == 1)
	{
		...
		if(DECT_Trembler_TestPass_Test(panel) != 1)
	{
		sprintf(test_number, "Trembler_TestPass");
		DECT_Clean_Shutdown_Sequence(panel);
		return 0;
	}
	...
	if(DECT_Get_HW_Version_Test (panel,1) != 1)
	{
		sprintf(test_number, "HW_Version_Test");
		DECT_Clean_Shutdown_Sequence(panel);
		return 0;
...
/*	if(DECT_Audio_Speaker_Test (panel) != 1){
	...
	if(DECT_Get_HW_Version_Test (panel,2) != 1)
	{
		sprintf(test_number, "HW_Version_Test");
		DECT_Clean_Shutdown_Sequence(panel);
		return 0;
